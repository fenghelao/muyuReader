...
npm run dist         # electron-builder 打包安装程序(M4,尚未验证）
```
...
src/main/                    Electron 主进程(Node,无 DOM)
  index.ts                   BaseWindow + 两 WebContentsView(reader/decoy)、老板键全局快捷键、标题锁死、IPC book:open
...
src/preload/index.ts         contextBridge 白名单:window.api.openBook();非 Electron 时 window.api 为 undefined
src/renderer/
  ...
  src/
    ...
    App.tsx                  布局 + 主题/字号 effect + Esc 老板键(仅浏览器)
    store.ts                 Zustand:mode/theme/font/books/blocks/blockIndex/messages/typing/bossOn + advance/loadBook/showError
    ...
    components/              Sidebar/TopBar/Thread/Composer/AssistantMessage/ThinkBlock/UserBubble/Decoy/DecoyScreen/DevBar/icons
...
- **老板键**:PROJECT_PLAN 把它排在 M3;已提前做完,且就是双 WebContentsView 方案。浏览器版(dev:web)用页面内 `Decoy` overlay + `Esc` 兜底,Electron 版用主进程切视图 + `Ctrl+Shift+Space`。
- **正文内容**:现在 `content.ts` 内置一段**原创武侠范文**当演示;真书走 M2 解析器。范文里的书名/章节标题**不注入正文**(§5.8 防露馅)。
- **阅读进度**:尚未持久化(见 §7);目前每次启动从演示内容 HISTORY=3 回填。

## 7. 下一步 Roadmap(建议顺序)

1. **M4 打包**:`npm run dist` 走通 electron-builder,双平台图标(build/claude.ico、claude.icns 待放)、mac 公证可选。验证任务管理器/⌘-Tab 显示名=Claude。
2. **阅读进度持久化**(§10):electron-store 存 `ReadingPosition{fileHash,chunkerVersion,blockIndex}` + 书架(书名/文件路径),续读回填历史(§5.6)。现在换书/重启即丢进度。**这是"能日常用"的关键缺口。**
3. **书架真实化**:现在 Recents 是内存里的假条目 + 导入时新增;需要把每本书的路径/进度落库,点 Recents 能切回不同书(现在切了不会重载内容)。
4. **PDF 复杂版式**:当前只做基础行聚类;多栏/表格重排待优化(§7.2)。
5. **MOBI/AZW3**:PROJECT_PLAN §3.4 的 foliate-js spike(高不确定,做前先评估)。
6. **微信读书书架只读同步**(§8,非首发):走官方 Agent Skill 拉书架填 Recents,正文仍本地。**绝不抓正文。**
7. **防偷看增强**(§9.5):失焦模糊、内容保护(仅 Windows 有效,mac 如实标注局限)。
8. **收尾伪装**:把临时的 `DevBar`(右下角排版/字号/亮暗/老板键)并进"外观设置"(伪装成 Claude 的 Settings)。
